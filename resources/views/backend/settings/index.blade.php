...
@push('after-scripts')
<script>
  // Try to inject a Maintenance link into the settings sidebar used by the Vue settings app.
  (function() {
    const targetUrl = "{{ route('backend.settings.maintenance') }}";

    function addLink() {
      const sidebar = document.querySelector('.setting-sidebar-inner, #setting-sidebar, .setting-sidebar, .setting-sidebar-panel, .list-group');
      if (!sidebar) return false;

      // Avoid duplicate
      if (sidebar.querySelector('#link-maintenance-injected')) return true;

      const wrapper = document.createElement('div');
      wrapper.className = 'mb-3 active-menu';
      wrapper.innerHTML = `<a id="link-maintenance-injected" href="${targetUrl}" class="btn btn-border">` +
        `<i class="fa-solid fa-tools"></i> {{ __('setting_sidebar.lbl_maintenance') }}` +
        `</a>`;

      // Insert near top
      if (sidebar.firstChild) sidebar.insertBefore(wrapper, sidebar.firstChild);
    ...
    const interval = setInterval(function() {
      attempts++;
      if (addLink() || attempts > 20) clearInterval(interval);
    }, 250);
  })();
</script>
@endhasPermission
