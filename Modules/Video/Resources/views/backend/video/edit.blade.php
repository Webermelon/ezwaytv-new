                ...
                <div class="card-body">
                    <div class="row gy-3">
                        <div class="col-md-6 col-lg-4">
                            ...
                            {{ html()->text('duration')->attribute('value', old('duration', normalizeVideoDurationForInput($data->duration)))->placeholder('HH:MM:SS or MM:SS')->class('form-control')->attribute('required', 'required')->attribute('pattern', '^(\\d{1,2}:)?[0-5]?\\d:[0-5]\\d$')->attribute('inputmode', 'numeric') }}
                        ...
                        <div class="col-md-6 col-lg-4">
                            ...
                            <select name="category_ids[]" id="category_ids" class="form-control select2" multiple="multiple" data-placeholder="Select categories...">
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->id }}" {{ in_array($cat->id, old('category_ids', $selectedCategoryIds ?? [])) ? 'selected' : '' }}>{{ $cat->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        @endif
...
@push('after-scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            ['file_url4', 'file_url_video', 'video_url_input'].forEach(function(id) {
                const input = document.getElementById(id);
                if (!input) return;
